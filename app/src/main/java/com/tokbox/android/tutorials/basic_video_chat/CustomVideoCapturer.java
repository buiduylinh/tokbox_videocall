...
public class CustomVideoCapturer extends BaseVideoCapturer implements
    ...
    public CustomVideoCapturer(Context context, Publisher.CameraCaptureResolution resolution,
        ...
        behaviorSubject.observeOn(Schedulers.computation()).subscribe(data -> {
            if (isCaptureRunning) {
                ...
                if (data.length == expectedFrameSize) {
                    ...
                    if (enableFilter) {
                        ...
                        int[] item = gpuImage.getBitmapForFilter(data, captureWidth, captureHeight);

                        if (metadataSource != null) {
                            byte[] framemetadata = metadataSource.retrieveMetadata();
                            provideIntArrayFrame(item, ARGB, captureWidth,
    ...
    public void enableFilter(boolean isEnable) {
        this.enableFilter = isEnable;
    }

    public void setBeautyLevel(float beautyLevel) {
        float beauty = beautyLevel * 2f - 100f;
        gpuImageBeautyFilter.setBeautyLevel(beauty / 100f);
    }

    public void setToneLevel(float toneLevel) {
        float beauty = toneLevel * 2f - 100f;
        gpuImageBeautyFilter.setToneLevel(beauty / 100f);
    }
    ...
    BehaviorSubject<byte[]> behaviorSubject = BehaviorSubject.create();
