...
JNIEXPORT void JNICALL
Java_jp_co_cyberagent_android_gpuimage_GPUImageNativeLibrary_getArrayPixel(JNIEnv *jenv,
                                                                           jclass thiz,
                                                                           jint width, jint height,
                                                                           jintArray out) {
    int i, j;
    jint *srcByteBuffer = (jint *) ((*jenv)->GetPrimitiveArrayCritical(jenv, out, 0));

    glReadPixels(0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, srcByteBuffer);
    ...
    (*jenv)->ReleasePrimitiveArrayCritical(jenv, out, pIntBuffer, 0);
}