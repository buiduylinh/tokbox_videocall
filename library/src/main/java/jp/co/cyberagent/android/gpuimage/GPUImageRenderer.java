...
public class GPUImageRenderer implements GLSurfaceView.Renderer, GLTextureView.Renderer, PreviewCallback {
    ...
    public void setImageByteBuffer(final byte[] data, final int imageWidth, final int imageHeight) {

        if (glRgbBuffer == null) {
            glRgbBuffer = IntBuffer.allocate(imageWidth * imageHeight);
        }
        ...
        runOnDraw(new Runnable() {
            ...
            @Override
            public void run() {
                GPUImageNativeLibrary.YUVtoARBG(data, imageWidth, imageHeight, glRgbBuffer.array());

                glTextureId = OpenGlUtils.loadTexture(glRgbBuffer, imageWidth, imageHeight, glTextureId);

                if (GPUImageRenderer.this.imageWidth != imageWidth) {
                    GPUImageRenderer.this.imageWidth = imageWidth;
                    GPUImageRenderer.this.imageHeight = imageHeight;
                    adjustImageScaling();
                }
            }
        });
